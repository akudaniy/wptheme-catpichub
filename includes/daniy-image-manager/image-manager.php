 ...
 * Last Change: 11:50 PM 22 Juli 2014
 */
...
define('THEME_IMAGE_FALLBACK', get_template_directory_uri() . '/includes/daniy-image-manager/image_default_fallback.jpg');
...
function imwp_get_thumbnail( $size='thumbnail', $image_class = NULL, $timb = array('script_file'=>'', 'params'=>'') ) {
  ...
  if( has_post_thumbnail( $post_ID ) ) {
    ...
    $post_title = $wpdb->get_var("SELECT post_title FROM {$table_prefix}posts WHERE ID = '{$post_ID}'");
...
function imwp_view_thumbnail( $size='thumbnail', $image_class = NULL, $timb = array('script_file'=>'', 'params'=>'') ) {
  echo imwp_get_thumbnail( $size, $image_class, $timb );
}
...
function imwp_get_image_fallback( $init = array() ) {
  ...
  $format = '<img src="%s" width="%s" height="%s" class="%s" alt="%s" title="%s" />';
  $img = sprintf(
          $format,
          $img_src,
          $ims[0],
